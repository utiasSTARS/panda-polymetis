""" Originally from fairo/perception. Modified with new features. """
...
class RealsenseAPI:
    ...
    def get_rgb(self, reverse_channels=False):
        """Returns a numpy array of [n_cams, height, width, RGBD]"""
        framesets = self._get_frames_no_align()
        num_cams = self.get_num_cameras()
        ...
        if reverse_channels:
            return np.ascontiguousarray(np.squeeze(rgb[:, :, :, ::-1]))
        else:
